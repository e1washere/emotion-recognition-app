...
data_dir = "/kaggle/input/kdef-npys"

# We assume:
#   train_images.npy  -> shape (N,224,224,1)
...
def preprocess_image(image, label):
    """
    1) Resize to 224x224
    2) If grayscale -> replicate to 3 ch
...
class SaveBestAboveThreshold(callbacks.Callback):
    ...
    """
    def __init__(self, threshold=0.60, save_path="best_kdef_tf.h5"):
        super().__init__()
...
save_best_callback = SaveBestAboveThreshold(threshold=0.60, save_path="best_kdef_tf.h5")
...
best_model = keras.models.load_model("best_kdef_tf.h5")
...
print(report)
with open("vgg16_kdef_tf_report.txt", "w") as f:
    f.write(report)
...
plt.title("KDEF (TensorFlow) - Confusion Matrix (%) [BEST MODEL]")
plt.xlabel("Predicted Label")
plt.ylabel("True Label")
plt.savefig("confusion_matrix_kdef_tf.png", dpi=300, bbox_inches='tight')
plt.show()

print("All done! Best model: best_kdef_tf.h5")
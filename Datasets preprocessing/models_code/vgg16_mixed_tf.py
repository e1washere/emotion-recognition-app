...
# We assume your .npy files:
#   train_images.npy  -> shape (N,224,224,1) or (N,224,224)
#   train_labels.npy  -> shape (N,)
#   test_images.npy   -> shape (M,224,224,1)
#   test_labels.npy   -> shape (M,)
...
num_classes = 7  # e.g. 7 classes: afraid, angry, disgusted, happy, neutral, sad, surprised
...
class SaveBestAboveThreshold(callbacks.Callback):
    ...
    Overwrites if val_accuracy is higher.
    """
    def __init__(self, threshold=0.60, save_path="best_mixed_tf.h5"):
        super().__init__()
...
save_best_callback = SaveBestAboveThreshold(threshold=0.60, save_path="best_mixed_tf.h5")
...
best_model = keras.models.load_model("best_mixed_tf.h5")
...
print(report)
with open("vgg16_mixed_tf_report.txt", "w") as f:
    f.write(report)
...
plt.title("Mixed (TensorFlow) - Confusion Matrix (%) [BEST MODEL]")
plt.xlabel("Predicted Label")
plt.ylabel("True Label")
plt.savefig("confusion_matrix_mixed_tf.png", dpi=300, bbox_inches='tight')
plt.show()

print("All done! Best model in best_mixed_tf.h5.")
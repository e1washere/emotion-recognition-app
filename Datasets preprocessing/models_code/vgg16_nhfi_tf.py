...
data_dir = "/kaggle/input/nhfi-npys"

train_images = np.load(os.path.join(data_dir, "train_images.npy"))  # (N,224,224,1)
train_labels = np.load(os.path.join(data_dir, "train_labels.npy"))  # shape (N,)
test_images  = np.load(os.path.join(data_dir, "test_images.npy"))   # shape (M,224,224,1)
test_labels  = np.load(os.path.join(data_dir, "test_labels.npy"))   # shape (M,)

num_classes = 8  # We have 8 emotions total for NHFI with "afraid"
print(f"Train images: {train_images.shape}, Train labels: {train_labels.shape}")
print(f"Test images: {test_images.shape}, Test labels: {test_labels.shape}")
print(f"Number of classes: {num_classes}")

# 0=angry,1=disgusted,2=happy,3=afraid,4=neutral,5=sad,6=surprised,7=contempt
...
class SaveBestAboveThreshold(callbacks.Callback):
    ...
    """
    def __init__(self, threshold=0.60, save_path="best_nhfi_tf.h5"):
        super().__init__()
...
save_best_callback = SaveBestAboveThreshold(threshold=0.60, save_path="best_nhfi_tf.h5")
...
print("=== PHASE 1: 5 epochs, conv base frozen ===")
model.fit(
    train_ds,
...
best_model = keras.models.load_model("best_nhfi_tf.h5")
...
plt.title("NHFI (TensorFlow) - Confusion Matrix (%) [BEST MODEL]")
plt.xlabel("Predicted Label")
plt.ylabel("True Label")
plt.savefig("confusion_matrix_nhfi_tf.png", dpi=300, bbox_inches='tight')
plt.show()

print("All done! Best model: best_nhfi_tf.h5")
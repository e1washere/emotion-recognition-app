...
num_classes = 8  # because we have 8 emotions including "afraid"
...
# 0=afraid,1=angry,2=disgusted,3=happy,4=neutral,5=sad,6=surprised,7=contempt
...
train_dataset = NHFIDataset(train_images, train_labels)
test_dataset  = NHFIDataset(test_images, test_labels)
...
best_model_path = "best_nhfi_pt.pth"
best_val_acc = 0.0
...
for epoch in range(phase1_epochs):
    ...
    print(f"Epoch [{epoch+1}/{phase1_epochs}]  Loss: {avg_loss:.4f}, Val_Acc: {val_acc:.4f}")
...
for epoch in range(phase2_epochs):
    ...
    val_acc = evaluate_accuracy(test_loader)
    avg_loss = running_loss / len(train_loader)
    print(f"Epoch [{epoch+1}/{phase2_epochs}]  Loss: {avg_loss:.4f}, Val_Acc: {val_acc:.4f}")
    save_best_model_if_needed(val_acc)
...
with open("vgg16_nhfi_pytorch_report.txt", "w") as f:
    f.write(report)
...
plt.title("NHFI (PyTorch) - Confusion Matrix (%) [BEST MODEL]")
plt.xlabel("Predicted Label")
plt.ylabel("True Label")
plt.savefig("confusion_matrix_nhfi_pt.png", dpi=300, bbox_inches='tight')
plt.show()

print("All done! Best model: best_nhfi_pt.pth")